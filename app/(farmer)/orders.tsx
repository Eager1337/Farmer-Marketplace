...
export default function FarmerOrdersScreen() {
  ...
  return (
    <View className="flex-1 bg-gray-50">
      <View className="px-6 pt-16 pb-4 bg-earth rounded-b-3xl flex-row items-center border-b border-earth shadow-sm">
        <TouchableOpacity 
          onPress={() => router.back()}
        ...
        <Text className="text-white text-xl font-bold">Farmer Orders</Text>
      </View>

      <View className="flex-row px-6 mt-4 mb-2">
        <TouchableOpacity 
          className={`flex-1 py-3 items-center border-b-2 ${activeTab === 'active' ? 'border-earth' : 'border-transparent'}`}
          onPress={() => setActiveTab('active')}
        >
          <Text className={`font-bold ${activeTab === 'active' ? 'text-earth' : 'text-gray-500'}`}>
            Active Orders ({activeOrders.length})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity 
          className={`flex-1 py-3 items-center border-b-2 ${activeTab === 'history' ? 'border-earth' : 'border-transparent'}`}
          onPress={() => setActiveTab('history')}
        >
          <Text className={`font-bold ${activeTab === 'history' ? 'text-earth' : 'text-gray-500'}`}>
